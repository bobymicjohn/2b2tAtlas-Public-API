...
The catalog-aware axis guard is limited to the known `+/-100,000` merged-background square. Outside that square, a long one-directional exhibit may be legitimate, so void closure remains authoritative and no direction, radius, or chunk-count fuse is applied. Named builds remain unbounded even inside the square and continue to require the normal confidence and neighboring-warp review.
